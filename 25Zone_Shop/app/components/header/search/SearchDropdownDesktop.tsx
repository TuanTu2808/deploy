...
export function SearchDropdownDesktop({
    open,
    anchorRef,
    onClose,
    onSubmit,
    ...
    const [pos, setPos] = useState<{ top: number; right: number; width: number }>({
        top: 90,
        right: 16,
        width: 640,
    });
    ...
    const syncPos = () => {
        ...
        const width = Math.min(720, Math.max(460, vw - 64));
    ...
    return (
        <div className={"hidden lg:block fixed inset-0 z-[75] " + (mounted ? "pointer-events-auto" : "pointer-events-none")}>
            <div
                className={
                    "absolute inset-0 transition-opacity duration-250 ease-[cubic-bezier(0.16,1,0.3,1)] " +
            ...
            <div
                ref={panelRef}
                className={
                    "fixed rounded-2xl border border-white/45 bg-white/90 backdrop-blur-xl overflow-hidden " +
                    "shadow-[0_34px_96px_rgba(0,0,0,0.22)] will-change-transform " +
                ...
                style={{ top: pos.top, right: pos.right, width: pos.width, maxHeight: "80vh", overflow: "auto" }}
                role="dialog"
                aria-modal="true"
                ...
                <div className="px-5 py-4 border-b border-gray-100/80 flex items-center justify-between sticky top-0 bg-white/95">
                    <div className="flex items-center gap-2">
                        ...
                        <div className="leading-tight">
                            <p className="font-extrabold text-[#003366] uppercase tracking-wide">Tìm kiếm</p>
                            <p className="text-xs text-gray-500">Tìm nhanh sản phẩm bạn muốn</p>
                        </div>
                    </div>
                ...
                >
                    <div className="relative">
                        ...
                        <input
                            ...
                            placeholder="Nhập từ khóa… (VD: sữa rửa mặt, dầu gội, sáp...)"
                            className="
