...
const normalizeDefaultAddress = async (
  ...
  if (!defaultId) {
    defaultId =
      rows.find((row) => Number(row.Is_default) === 1)?.Id_address_ship || null;
  }
  if (!defaultId) {
...
router.put("/me", requireAuth, async (req, res) => {
  try {
    ...
    const address =
      normalizeText(req.body?.address) || current.Address || "Chưa cập nhật";

    if (!name || !phone || !email) {
      return res
        .status(400)
        .json({ message: "Tên, email, số điện thoại không được để trống." });
    }
    ...
    if (existsRows.length) {
      return res
        .status(409)
        .json({ message: "Email hoặc số điện thoại đã được dùng." });
    }
...
router.post("/me/addresses", requireAuth, async (req, res) => {
  ...
  try {
    ...
    if (!receiverName || !phone || !province || !ward || !addressDetail) {
      return res
        .status(400)
        .json({ message: "Vui lòng nhập đầy đủ thông tin địa chỉ." });
    }

    if (phone.length < 9 || phone.length > 11) {
      return res
        .status(400)
        .json({ message: "Số điện thoại địa chỉ không hợp lệ." });
    }
    ...
    if (finalDefault) {
      await connection.query(
        "UPDATE Address_Ship SET Is_default = 0 WHERE Id_user = ?",
        [userId],
      );
    }
