...
export default function DanhMucPage() {
  ...
  const fetchDanhMuc = useCallback(async () => {
    try {
      setLoading(true);
      const res = await authorizedFetch(`${API_BASE}/api/danhmucsanpham`);
      if (!res.ok) {
  ...
  const handleAddCategory = async () => {
    ...
    try {
      const response = await authorizedFetch(`${API_BASE}/api/danhmucsanpham`, {
        method: "POST",
        headers: {
  ...
  const handleToggleStatus = async (id: number) => {
    try {
      const response = await authorizedFetch(`${API_BASE}/api/danhmucsanpham/${id}/status`, {
        method: "PATCH",
      });
  ...
  const handleEdit = async (item: DanhMuc) => {
    const newName = prompt("Nhập tên danh mục mới:", item.Name_category);
    if (!newName) return;
    ...
    try {
      const response = await authorizedFetch(
        `${API_BASE}/api/danhmucsanpham/${item.Id_category_products}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
          },
          body: JSON.stringify({ Name_category: newName.trim() }),
        }
      );
  ...
  return (
    <main className="flex-1 bg-[#F8FAFC] py-8 px-[100px]">
      <div className="mb-6 flex items-start justify-between">
        <div>
          <h2 className="text-2xl font-bold text-[#0F172A]">Quản lý Danh mục Sản phẩm</h2>
          <p className="mt-1 text-sm text-[#64748B]">
            Sắp xếp và quản lý phân loại sản phẩm cho cửa hàng.
          </p>
        </div>
          ...
          <table className="w-full text-sm">
            <thead className="bg-[#F8FAFC] text-[#94A3B8]">
              <tr className="text-left">
                ...
                <th className="px-6 py-4 text-center font-medium">Số lượng sản phẩm</th>
            ...
            <tbody className="divide-y divide-slate-200">
              {loading ? (
                ...
                currentData.map((item) => (
                  <tr key={item.Id_category_products} className="hover:bg-gray-50">
                    <td className="px-6 py-4 font-semibold text-[#0F172A]">{item.Name_category}</td>

                    <td className="px-6 py-4 text-center">
                      <span className="rounded-full bg-blue-50 px-3 py-1 text-xs font-medium text-blue-600">
                        {item.total_products ?? 0} sản phẩm
                      </span>
                    </td>
                    ...
                    <td className="px-6 py-4 text-center">
                      <div className="flex justify-center gap-2">
                        ...
                        <button
                          onClick={() => handleToggleStatus(item.Id_category_products)}
